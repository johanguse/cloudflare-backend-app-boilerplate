-- Initial schema. Table order satisfies SQLite foreign-key creation. If you regenerate with
-- `drizzle-kit generate`, verify CREATE TABLE order before applying (Drizzle may emit a different order).
CREATE TABLE `user` (
...
--> statement-breakpoint
CREATE INDEX `idx_file_uploads_user` ON `file_uploads` (`user_id`);--> statement-breakpoint
CREATE INDEX `idx_file_uploads_storage_key` ON `file_uploads` (`storage_key`);--> statement-breakpoint
